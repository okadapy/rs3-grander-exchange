...
// ws upgrades the connection. Token supplied via ?token=...
func (h *Handler) ws(c *gin.Context) {
	...
	client := &hub.Client{
		Hub: h.h, Conn: conn, Send: make(chan []byte, 64),
...
// onMessage handles subscribe/unsubscribe and chat.
func (h *Handler) onMessage(c *hub.Client, msg []byte) {
	...
	case "chat":
		if !h.chat.Allow(c.UserID) {
			c.Send <- []byte(`{"type":"error","payload":{"message":"rate limited (3s)"}}`)
			return
		}
